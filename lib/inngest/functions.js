    ...
    for (const budget of budgets) {
      ...
      await step.run(`check-budget-${budget.id}`, async () => {
        ...
        if (
          ...
          // FIX: only update lastAlertSent once, and only if email succeeded
          if (result.success) {
...
export const triggerRecurringTransactions = inngest.createFunction(
  ...
  { cron: "0 0 * * *" }, // FIX: removed trailing space
  async ({ step }) => {
...
export const processRecurringTransactions = inngest.createFunction(
  {
    id: "process-recurring-trasaction",
    throttle: {
      limit: 10,
      period: "1m",
  ...
  async ({ event, step }) => {
    ...
    await step.run("process-transaction", async () => {
      ...
      await db.$transaction(async (tx) => {
        // FIX: changed `date` to `data`, and added recurringInterval
...
function isTransactionDue(transaction) {
  ...
  // FIX: was `newDue` declared but `nextDue` used — unified to `nextDue`
